...
*/
        ...
        function handleClick(e){
            ...
            window.localStorage.setItem("id", JSON.stringify(e.id));
            window.localStorage.setItem("labels", JSON.stringify(e.labels));
            document.getElementById('hehehe').click()
            // document.getElementById('hehehe').empty()
        }
        function keyHandler() {
        ...
        var graphView = neo.graphView()
            .style(styleSheet)
            .width($container.width()).height($container.height()).on('nodeClicked', (e, index)=>handleClick(e, index)).on('relationshipClicked', dummyFunc).on('nodeDblClicked', dummyFunc);
